...
// The following solution comes from this blogpost: https://adamrackis.dev/blog/swift-codable-any.
...
func runEncode(_ json: JSON) -> String {
    let encoder = JSONEncoder()
    if let result = try? encoder.encode(json) {
        return String(data: result, encoding: .utf8) ?? ""
    }
    return ""
}

func runDecode(_ json: String) -> JSON? {
...
BDOUser?) -> Void) async {
        ...
        await Network.registerBDO(baseURL: ServiceURLs.bdo.rawValue, bdo: bdo, callback: handler)
    }
